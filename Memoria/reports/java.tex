...
Por otro lado nos encontramos con que en Java el paso de parámetros es siempre por valor y no por referencia, lo que ha ocasionado problemas que no existían en Pascal para reestructurar el árbol al hacer una eliminación (explicaremos estos problemas más adelante cuando entremos en detalles de la implementación). \\
...
\begin{itemize}
	...
	\item \texttt{tABB} se convierte en la clase Bst\footnote{Binary Search Tree}, que tiene un atributo \texttt{Node} representando la raíz del árbol. Todas las funciones y procedimientos definidos en el TAD de Pascal son métodos de esta clase.
\end{itemize}
De este modo, Bst actúa como un envoltorio y punto de acceso a la estructura de nodos. Hacerlo de esta forma nos permite realizar operaciones como la eliminación de la raíz.\\ % Explicar después
